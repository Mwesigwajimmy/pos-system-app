...
// AUTHORITATIVE VERSION: 1.4.3 (Loop-Proof Identity Weld)
...
// --- CONFIGURATION (Original undisturbed) ---
...
export async function middleware(request: NextRequest) {
    ...
    const isPublicPathForBot = publicPaths.some(pp => pathWithoutLocale === pp || pathWithoutLocale.startsWith(`${pp}/`));

    if (isBot && isPublicPathForBot) return response;
    ...
    // --- SUBSCRIPTION SECURITY WELD (FIXED) ---
    ...
    // FIX: Redirection destination must be 'defaultDashboard' to avoid generic /dashboard loop
