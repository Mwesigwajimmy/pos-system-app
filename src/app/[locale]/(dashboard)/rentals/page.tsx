...
// Fetches all properties along with their nested rental units
...
async function addProperty(data: { name: string; address: string; property_type: string; business_id: any; }) {
    const supabase = createClient();
    const { error } = await supabase.from('properties').insert(data);
    if (error) throw error;
}

// Adds a new rental unit to a specific property
...
export default function PropertiesPage() {
    ...
    // THIS IS THE CORRECTED FUNCTION WITH DEBUGGING LOGS
    const handlePropertySubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        console.log("--- Save Property button clicked ---"); // Log #1
        const fd = new FormData(e.currentTarget);
        const supabase = createClient();

        try {
            // 1. Get the current user
            console.log("Step 1: Getting current user..."); // Log #2
            const { data: { user } } = await supabase.auth.getUser();
            if (!user) {
                console.error("DEBUG: No user found. User must be logged in."); // Log #3
                throw new Error("You must be logged in to save a property.");
            }
            console.log("Step 1 SUCCESS: User found with ID:", user.id); // Log #4

            // 2. Get their profile to find their business_id
            console.log("Step 2: Getting user profile to find business_id..."); // Log #5
            ...
            if (profileError) {
                console.error("DEBUG: Error fetching profile:", profileError); // Log #6
                throw new Error(`Could not find your user profile: ${profileError.message}`);
            }

            console.log("Step 2 SUCCESS: Profile data received:", profile); // Log #7

            if (!profile || !profile.business_id) {
                console.error("DEBUG: Profile found, but business_id is missing or null.", profile); // Log #8
                throw new Error("Could not find a business for your account. Please check your profile settings.");
            }
            
            console.log("Step 2 SUCCESS: Found business_id:", profile.business_id); // Log #9

            // 3. Prepare data for mutation
            const propertyData = {
                name: fd.get('name') as string,
                address: fd.get('address') as string,
                property_type: fd.get('type') as string,
                business_id: profile.business_id
            };

            console.log("Step 3: Calling mutation with this data:", propertyData); // Log #10

            // 4. Call the mutation
            propertyMutation.mutate(propertyData);

        } catch (error: any) {
            console.error("--- An error occurred in handlePropertySubmit ---", error); // Log #11
    ...
    const handleUnitSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        unitMutation.mutate({
            ...
            unit_identifier: fd.get('unit_identifier') as string,
