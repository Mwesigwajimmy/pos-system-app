...
export default async function PaymentsPage({ params: { locale } }: PageProps) {
    ...
    return (
      <div className="flex flex-col h-[70vh] items-center justify-center p-6 text-center animate-in fade-in duration-500">
        <div className="bg-red-50 p-12 rounded-[40px] border-2 border-dashed border-red-200 max-w-md shadow-2xl shadow-red-500/10">
          <ShieldCheck className="h-16 w-16 text-red-600 mx-auto mb-6" />
  ...
  /**
   * DEEP WELD: Session Injection for RLS
   * We tell the database session which tenant we are auditing before running queries.
   */
  await supabase.rpc('set_config', { 
    name: 'app.current_business_id', 
    value: bizId 
  });
