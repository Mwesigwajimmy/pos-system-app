...
export default async function PaymentsPage({ params }: PageProps) {
      ...
      <div className="flex flex-col h-[80vh] items-center justify-center p-6 text-center animate-in fade-in duration-700">
        <div className="bg-rose-50 p-12 rounded-[40px] border-2 border-dashed border-rose-200 max-w-md shadow-2xl">
          ...
          <p className="text-rose-700 mt-4 font-medium leading-relaxed uppercase text-[10px] tracking-widest">
            Verification failure: Profile is not anchored to a valid Business Identity.
          </p>
          <Button variant="destructive" className="mt-8 font-bold uppercase text-[11px] tracking-widest px-10 h-12 rounded-2xl shadow-lg" asChild>
            <Link href={`/${locale}/dashboard`}>Return to Base</Link>
          </Button>
        </div>
      </div>
    );
  ...
  const [invoicesRes, accountsRes] = await Promise.all([
    ...
    supabase
      ...
      .eq("business_id", ctx.res_biz_id)
      .eq("code", "1000") // Bank/Cash clearing pool location
      .eq("is_active", true)
  ]);

  // 6. INFRASTRUCTURE AUDIT RESPONSE
  if (!ctx.is_ledger_ready) {
    return (
        <div className="flex flex-col h-[80vh] items-center justify-center p-6 text-center animate-in zoom-in-95 duration-500">
            <div className="bg-amber-50 p-12 rounded-[40px] border-2 border-dashed border-amber-200 max-w-lg shadow-2xl">
                ...
                <p className="text-amber-800 text-[11px] font-bold uppercase tracking-widest mt-4 leading-relaxed">
                    The General Ledger for {ctx.res_biz_name} is missing core components (GEN/1210).
                </p>
                <div className="mt-8">
                    <Button className="px-10 h-14 bg-amber-600 hover:bg-amber-700 text-white rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-lg" asChild>
  ...
  return (
    <div className="container mx-auto py-10 max-w-7xl px-6 animate-in fade-in duration-1000">
      ...
      {/* MASTER HEADER - Straight Typography Protocol */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 font-bold mt-1 uppercase text-[11px] tracking-widest leading-none ml-1">
                Autonomous Settlement Handshake for <span className="text-emerald-600 font-black underline underline-offset-4 decoration-emerald-500/30">{ctx.res_biz_name}</span>
              </p>
            </div>
          </div>
        </div>
           ...
           <div className="p-8 bg-slate-900 rounded-[32px] text-white shadow-2xl relative overflow-hidden group h-fit">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <p className="text-[13px] text-slate-300 font-medium leading-relaxed uppercase tracking-tight">
                    Settlements are anchored to Account 1210. Mathematical parity is guaranteed by the {ctx.res_currency} database kernel.
                </p>
           ...
           <div className="p-6 bg-white border border-slate-200 rounded-[32px] shadow-sm flex items-center gap-4 group hover:border-blue-500 transition-colors">
              ...
              <div>
                ...
                <p className="text-[10px] font-mono font-bold text-slate-900 mt-1 uppercase">
                    {String(ctx.res_biz_id).substring(0,18)}
                </p>
              </div>
           </div>
        </div>
