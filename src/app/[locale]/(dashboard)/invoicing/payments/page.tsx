...
export default async function PaymentsPage({ params }: PageProps) {
  // 1. NEXT.JS 15 COMPATIBILITY: Securely resolve routing and store
  ...
  // 3. SYSTEM-ALIGNED IDENTITY RESOLUTION
  // Based on your RPC 'function_body', the source of truth is 'public.profiles'
  // Columns verified: business_id, full_name, business_name
  const { data: profile, error: profileError } = await supabase
    .from("profiles")
    .select("business_id, full_name, business_name")
    .eq("id", user.id)
    .maybeSingle();

  // Resolve ID and Name using the exact system-standard keys
  const bizId = profile?.business_id;
  const activeBusinessName = profile?.business_name || profile?.full_name || "Sovereign Unit";

  // 4. SECURITY GATEKEEPER
  // This triggers the UI 'Identity Lock' if the user row exists but isn't linked to a bizId
      ...
      <div className="flex flex-col h-[80vh] items-center justify-center p-6 text-center animate-in fade-in duration-700">
        <div className="bg-rose-50 p-12 rounded-[40px] border-2 border-dashed border-rose-200 max-w-md shadow-2xl shadow-rose-500/10">
          ...
          <p className="text-rose-700 mt-4 font-medium leading-relaxed uppercase text-[10px] tracking-widest">
            Forensic Failure: Profile is not anchored to a Business Unit in the Ledger.
          </p>
  ...
  // 5. DATA ACQUISITION (RLS-SECURED)
  // Fetching from confirmed 'business_id' anchor in Invoices and Accounts
  const [invoicesRes, accountsRes] = await Promise.all([
    ...
    supabase
      ...
      .eq("business_id", bizId)
      .eq("code", "1000") // Clearing pool discovery code verified in SQL
      .eq("is_active", true)
  ]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 font-bold mt-1 uppercase text-[11px] tracking-widest leading-none ml-1">
                Settlement Handshake for <span className="text-emerald-600 font-black underline underline-offset-4 decoration-emerald-500/30">{activeBusinessName}</span>
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="space-y-4">
                    <p className="text-[13px] text-slate-300 font-medium leading-relaxed">
                        Settlements recorded trigger a handshake between Account <span className="text-white font-bold underline underline-offset-8 decoration-blue-500">1210</span> and chosen liquidity.
                    </p>
                </div>
