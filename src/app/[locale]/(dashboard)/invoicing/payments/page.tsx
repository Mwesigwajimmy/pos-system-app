...
import { Alert, AlertDescription, AlertTitle } from "@/components/alert"; // UI Path Weld
...
export default async function PaymentsPage({ params: { locale } }: PageProps) {
  ...
  try {
      ...
      return (
        <div className="flex flex-col h-[70vh] items-center justify-center p-6 text-center animate-in fade-in duration-500">
          <div className="bg-red-50 p-12 rounded-[40px] border-2 border-dashed border-red-200 max-w-md shadow-2xl shadow-red-500/10">
            ...
            <Link href={`/${locale}/dashboard`} className="mt-8 inline-block px-10 h-12 bg-red-600 text-white rounded-2xl font-bold uppercase text-[11px] tracking-widest hover:bg-red-700 transition-all active:scale-95">Return to Command Center</Link>
          </div>
        </div>
      );
    }
    ...
    // DEEP WELD: Handle duplicate GEN journals safely using limit(1)
    ...
    // DEEP WELD: Ensure Centenary Bank (Asset/bank) is fetched for the destination dropdown
    const [invoicesRes, accountsRes] = await Promise.all([
      ...
      supabase
        ...
        .eq("business_id", bizId)
        .eq("type", "Asset")
        .in("subtype", ["bank", "Bank", "cash", "Cash"])
        .eq("is_active", true)
    ]);

    // Error handling for table-specific queries
    if (invoicesRes.error || accountsRes.error) {
        throw new Error(invoicesRes.error?.message || accountsRes.error?.message);
    }
               ...
               <div className="p-6 bg-amber-50 border border-amber-200 rounded-[28px] flex gap-4 items-start shadow-sm animate-in slide-in-from-right-4">
                 ...
                 <div className="space-y-1">
                   ...
                   <p className="text-[10px] text-amber-700 font-medium leading-relaxed mt-1 uppercase tracking-tight">
                     No active recipient bank account found. Please register Bank assets to finalize settlements.
                   </p>
                 </div>
               </div>
             )}
    ...
    // SOVEREIGN SAFETY CATCH: Prevents the generic 'Server Exception' screen
    return (
      <div className="p-8 max-w-3xl mx-auto flex h-[80vh] items-center justify-center">
        <div className="bg-red-600 text-white rounded-[32px] p-10 shadow-2xl w-full">
          ...
          <div className="mt-6 font-mono text-[10px] p-3 bg-red-700/50 rounded-xl uppercase tracking-widest overflow-hidden truncate">
            Technical Root: {error.message}
          </div>
          <Link href={`/${locale}/invoicing/payments`} className="mt-8 inline-flex items-center px-8 h-12 bg-white text-red-600 rounded-2xl font-bold uppercase text-[10px] tracking-widest hover:bg-slate-100 transition-all">
