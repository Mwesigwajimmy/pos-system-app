...
async function fetchEmployees(pagination: PaginationState): Promise<{ employees: any[], total_count: number }> {
...
export default function EmployeeDataTable<TData, TValue>({ columns }: DataTableProps<TData, TValue>) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end"><AddEmployeeDialog /></div>
      ...
      <div className="flex items-center justify-end space-x-2">
        <span className="flex-1 text-sm text-muted-foreground">Total Employees: {data?.total_count ?? 0}</span>
