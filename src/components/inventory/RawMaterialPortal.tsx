...
export default function RawMaterialPortal() {
  ...
  // 4. DATA: Suppliers (Linked to suppliers table)
  ...
  const handleOnboard = async () => {
    if (!form.name || !form.uom_id) return toast.error("Please fill in the material name and unit.");
    setLoading(true);
    try {
  ...
  const logAdjustment = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.rpc('process_stock_adjustment_v2', {
        ...
        p_reason: `Stock Correction: ${adjustData.reason}`
      });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const downloadReport = (format: 'PDF' | 'EXCEL') => {
    if (format === 'EXCEL') {
        ...
        link.href = url;
        link.download = `Material_Report_${Date.now()}.csv`;
        link.click();
        return;
    }
    const doc = new jsPDF('p', 'mm', 'a4');
    doc.setFontSize(20);
    doc.text("RAW MATERIAL INVENTORY REPORT", 14, 22);
    doc.setFontSize(10);
    doc.text(`Business: ${businessName} | Date: ${new Date().toLocaleDateString()}`, 14, 30);
    (doc as any).autoTable({
        startY: 40,
        head: [['Material Name', 'SKU', 'Current Stock', 'Unit Price', 'Total Value']],
    ...
    doc.save(`Material_Report_${Date.now()}.pdf`);
  };

  if (isLoading) return <div className="flex flex-col items-center justify-center min-h-screen bg-white gap-4"><Loader2 className="animate-spin text-blue-600 h-10 w-10" /><p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Accessing Material Registry...</p></div>;
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 border-b border-slate-100 pb-10">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-blue-600 font-bold text-[10px] uppercase tracking-[0.2em]">
                <Activity size={16} /> Inventory Management
            </div>
            ...
            <div className="flex items-center gap-8 bg-white px-6 py-4 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-blue-600 tabular-nums">
                    {ledgerStats.totalValuation.toLocaleString()} <span className="text-[10px] font-medium text-slate-400">{businessCurrency}</span>
                </p>
              </div>
            </div>
        ...
        <Card className="border border-slate-200 shadow-sm rounded-3xl overflow-hidden bg-white">
          <CardHeader className="px-8 py-6 border-b border-slate-100 bg-slate-50/30">
            <div className="flex items-center gap-3">
               ...
               <CardTitle className="text-md font-bold text-slate-900 uppercase tracking-wider">New Material Enrollment</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              <div className="lg:col-span-2 space-y-2">
                <Label className="text-[11px] font-bold text-slate-400 uppercase ml-1">Material Name</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[11px] font-bold text-slate-400 uppercase ml-1">SKU / ID Code</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[11px] font-bold text-slate-400 uppercase ml-1">Quality Grade</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[11px] font-bold text-slate-400 uppercase ml-1">Measurement Unit</Label>
                <div className="flex gap-2">
                  <Select value={form.uom_id} onValueChange={v => setForm({...form, uom_id: v})}>
                    <SelectTrigger className="h-11 rounded-xl flex-1"><SelectValue placeholder="Metric" /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="space-y-2">
                <Label className="text-[11px] font-bold text-slate-400 uppercase ml-1">Unit Cost ({businessCurrency})</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[11px] font-bold text-blue-600 uppercase ml-1">Initial Stock Level</Label>
                <Input type="number" value={form.qty} onChange={e => setForm({...form, qty: Number(e.target.value)})} className="h-11 border-blue-100 bg-blue-50/30 rounded-xl font-bold text-right text-blue-700" />
              </div>
              
              <div className="lg:col-span-3 space-y-2">
                <Label className="text-[11px] font-bold text-slate-400 uppercase ml-1">Verified Supplier</Label>
                <Select onValueChange={v => setForm({...form, supplier_id: v})}>
                  <SelectTrigger className="h-11 rounded-xl"><SelectValue placeholder="Identify Supply Partner" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex items-end">
                <Button onClick={handleOnboard} disabled={loading} className="w-full h-11 bg-slate-900 hover:bg-black text-white font-bold rounded-xl shadow-lg transition-all uppercase tracking-widest text-[10px]">
                  {loading ? <Loader2 className="animate-spin h-5 w-5" /> : "Complete Enrollment"}
                </Button>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="border border-slate-200 shadow-sm rounded-3xl overflow-hidden bg-white">
          <CardHeader className="px-8 py-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/20">
            <div className="space-y-0.5">
              <CardTitle className="text-xl font-bold text-slate-900">Current Material Ledger</CardTitle>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Real-time inventory levels and valuations</p>
            </div>
              ...
              <Table>
                <TableHeader className="bg-slate-50">
                  <TableRow className="h-14 border-none">
                    ...
                    <TableHead className="px-10 text-right font-bold uppercase text-slate-500 text-[10px] tracking-widest">Total Valuation</TableHead>
                  ...
                  {materials?.filter(m => m.product_name.toLowerCase().includes(searchTerm.toLowerCase())).map(m => (
                    <TableRow key={m.variant_id} className="h-20 hover:bg-slate-50/50 transition-colors border-b last:border-none">
                      ...
                      <TableCell className="px-6">
                         {m.current_stock > 50 ? (
                           <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-none font-bold text-[9px] uppercase px-3 py-1 rounded-md">Operational</Badge>
                         ) : (
                           <Badge variant="outline" className="bg-rose-50 text-rose-700 border-none font-bold text-[9px] uppercase px-3 py-1 rounded-md">Restock</Badge>
                         )}
                      </TableCell>
                      <TableCell>
                          ...
                          <DialogContent className="max-w-md rounded-3xl p-0 overflow-hidden border-none shadow-3xl bg-white">
                            ...
                            <div className="p-10 space-y-8">
                                <div className="space-y-4">
                                  <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center block">Actual Count to Deduct</Label>
                                  <div className="relative">
                                    <Input type="number" className="h-20 border-slate-100 bg-slate-50 font-black text-4xl text-center rounded-2xl shadow-inner text-slate-900" onChange={e => setAdjustData({...adjustData, qty: Number(e.target.value)})} />
                                    <span className="absolute right-6 top-1/2 -translate-y-1/2 font-bold text-slate-300 uppercase text-[11px]">{m.unit}</span>
                                  </div>
                                </div>
                                <div className="space-y-2">
                                  <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Adjustment Category</Label>
                                  <Select onValueChange={v => setAdjustData({...adjustData, reason: v})}>
                                    <SelectTrigger className="h-12 border-slate-200 bg-white font-semibold rounded-xl text-sm"><SelectValue placeholder="Identify Reason" /></SelectTrigger>
                                    <SelectContent>
                                      <SelectItem value="Waste">Production Waste</SelectItem>
                                      <SelectItem value="Damage">Material Damage</SelectItem>
                                      <SelectItem value="Audit">Physical Audit Correction</SelectItem>
                                    </SelectContent>
                                  </Select>
                                </div>
                            </div>
      ...
      <Dialog open={isUnitModalOpen} onOpenChange={setIsUnitModalOpen}>
          <DialogContent className="max-w-sm rounded-[2rem] p-0 overflow-hidden border-none shadow-3xl bg-white">
              ...
              <DialogFooter className="px-8 py-6 bg-slate-50 border-t flex gap-4">
                  <Button variant="ghost" onClick={() => setIsUnitModalOpen(false)} className="h-11 font-bold text-slate-400 uppercase text-[10px] tracking-widest">Discard</Button>
      ...
      <footer className="mt-16 flex flex-col md:flex-row items-center justify-between border-t border-slate-100 pt-10 pb-16 opacity-30">
          <div className="flex items-center gap-6">
             <div className="flex items-center gap-3">
                <ShieldCheck size={16} />
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]">System Registry Node v2.6.4</span>
             </div>
             <div className="h-4 w-px bg-slate-200 hidden md:block" />
             <span className="text-[10px] font-bold uppercase tracking-[0.3em]">Corporate Inventory Control</span>
          </div>
          <div className="flex items-center gap-3 mt-6 md:mt-0">
             <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
             <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Registry Sync: Active</span>
          </div>
      </footer>
    </div>
  );
