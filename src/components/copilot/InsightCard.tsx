 ...
 * 
 * Features: High-Density AreaCharts, Multi-Sector Metadata, 
 * Forensic Verification Badges, and Executive AI Command Hooks.
 * 
 * Integrity Grade: OMEGA-LEVEL / Forensic Ready.
 */
...
interface SuggestedAction {
  label: string;
  prompt: string;
  agent_id?: 'AURA-CFO' | 'AURA-COO' | 'AURA-HR' | 'AURA-PM';
}
...
export interface Insight {
  ...
  sector?: 'finance' | 'medical' | 'sacco' | 'logistics' | 'hr'; // Sector awareness
  message: string;
...
export default function InsightCard({ insight, onAskAI }: InsightCardProps) {
  ...
  const mutation = useMutation({
      ...
      onSuccess: () => {
          toast.success("Intelligence Acknowledged", {
            description: "Aura has archived this insight to long-term memory."
          });
  ...
  return (
    <motion.div 
      ...
      className={cn(
        'group relative break-inside-avoid rounded-2xl border-l-4 bg-card shadow-sm transition-all duration-300 hover:shadow-xl flex flex-col overflow-hidden mb-6', 
        config.borderColor,
        config.glow,
        'hover:-translate-y-1'
      )}
    >
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-3">
            <div className="flex flex-col gap-1">
                <CardTitle className={cn('flex items-center gap-2 text-sm font-bold uppercase tracking-tight', config.color)}>
                  <config.Icon className="h-4 w-4 flex-shrink-0" />
                  <span>{insight.title}</span>
                </CardTitle>
                <div className="flex items-center gap-2 mt-1">
                    {insight.is_forensic_verified && (
                        <div className="flex items-center gap-1 text-[9px] font-black text-emerald-600 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20 uppercase tracking-widest">
                            <ShieldCheck size={10} /> Forensic Verified
                        </div>
                    )}
                    <span className="text-[9px] font-mono text-slate-400 uppercase tracking-widest">
                        {insight.generated_at ? new Date(insight.generated_at).toLocaleTimeString() : 'Real-time Intel'}
                    </span>
                </div>
            </div>
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                    ...
                    <DropdownMenuItem onClick={() => onAskAI(`Explain this forensic insight in detail: ${insight.message}`)} className="cursor-pointer">
                        <TrendingUp className="mr-2 h-4 w-4 text-blue-500" /> Drill Down Logic
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={handleDismiss} className="cursor-pointer focus:bg-destructive/10 focus:text-destructive text-destructive">
                        <XCircle className="mr-2 h-4 w-4" /> Clear Notification
                    </DropdownMenuItem>
                </DropdownMenuContent>
            </DropdownMenu>
        </div>
      ...
      <CardContent className="space-y-4 flex-grow">
        ...
        {/* Visual Data Handshake */}
        {insight.chart_data && insight.chart_data.length > 0 && (
            <div className="h-32 -mx-2 bg-slate-50/50 dark:bg-slate-900/20 rounded-xl border border-dashed border-slate-100 dark:border-slate-800 p-2">
        ...
        {/* Forensic Key Data Points */}
        {insight.key_data && (
          <div className="grid grid-cols-1 gap-2 rounded-xl border bg-muted/30 p-3 shadow-inner">
            {Object.entries(insight.key_data).map(([key, item]) => (
              <div key={key} className="flex justify-between items-center group/item">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-bold capitalize text-slate-500">{key.replace(/_/g, ' ')}</span>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    {item.href ? (
                        <Button asChild variant="link" className="p-0 h-auto font-mono text-[11px] text-primary hover:text-blue-700 decoration-dotted underline-offset-4">
      ...
      {insight.suggested_actions && insight.suggested_actions.length > 0 && (
        <CardFooter className="flex flex-col items-start gap-2 pt-4 border-t bg-slate-50/50 dark:bg-slate-950/20 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 gap-2 w-full">
                ...
                <Button
                    ...
                    size="sm"
                    variant="outline"
                    className="group/btn relative w-full h-auto justify-between text-left py-2.5 px-4 rounded-xl border-slate-200 hover:border-primary hover:bg-white transition-all overflow-hidden"
                ...
                >
                    <div className="flex items-center gap-3 relative z-10">
                        ...
                        <span className="text-[11px] font-bold text-slate-700 group-hover/btn:text-slate-900">{action.label}</span>
                    </div>
      ...
      {/* FOOTER METADATA */}
      <div className="px-6 py-2 bg-slate-900 border-t border-white/5 flex justify-between items-center">
            <div className="flex items-center gap-1.5">
                <Fingerprint size={10} className="text-slate-600" />
                <span className="text-[8px] font-mono text-slate-600 uppercase tracking-tighter">Sovereign Link: {insight.id.slice(0, 8)}</span>
            </div>
            <div className="flex items-center gap-1">
                <SearchSlash size={10} className="text-slate-600" />
                <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest italic">BBU1 Protocol v10.8</span>
            </div>
      </div>
    </motion.div>
  );
...
/**
 * STATUS: Intelligence Interface Active.
