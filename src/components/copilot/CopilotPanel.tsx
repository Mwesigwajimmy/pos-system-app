...
/**
 * Physically triggers a browser download for base64 data generated by Aura
 */
...
/**
 * Renders the "Thinking Process" of the AI.
 * Handles specialized UI actions like Navigation and Downloads.
 */
const AgentStep = ({ data }: { data: any }): React.ReactNode => {
  ...
  } catch (e) { /* Non-JSON output handled as tool execution below */ }

  if (data.tool) {
    return (
      <div className="text-[10px] text-muted-foreground ml-11 my-2 p-3 border rounded-xl bg-slate-50 border-dashed animate-pulse">
        <div className="flex items-center gap-2">
          <Cog className="h-3 w-3 animate-spin text-slate-400" />
          <div>
            <p className="font-bold uppercase tracking-widest text-slate-500">Aura Core: {data.tool}</p>
      ...
      <div className="p-6 border-t bg-white shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.02)]">
        <form 
          onSubmit={(e) => {
            e.preventDefault();
            if (!isLinked) {
              toast.error("Handshake Failed: Identity context is missing.");
              return;
            }
        ...
        {/* Multi-Tenant Handshake Status (Forensic Verification) */}
        <div className="flex justify-between items-end mt-4 px-1">
            <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-2 text-[9px] uppercase tracking-tighter text-muted-foreground font-bold">
                    ...
                    Business ID: <span className="font-mono text-emerald-600 bg-emerald-50 px-1 rounded">{businessId ? businessId.slice(0, 18) : 'IDENTIFYING...'}</span>
                </div>
                <div className="flex items-center gap-2 text-[9px] uppercase tracking-tighter text-muted-foreground font-bold">
