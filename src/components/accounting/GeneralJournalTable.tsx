...
interface JournalTransaction {
  ...
  description: string;
  state: string;
  // Optional: present if your `accounting_transactions` table has a column
  // recording where the entry came from. Since the query below selects `*`,
  // this is picked up automatically if the column exists — rename it here
  // if your column uses a different name (e.g. `origin`, `entry_source`).
  source_type?: string;
  lines: JournalLine[];
}
...
const SOURCE_TYPES = [
  ...
  { value: 'invoice', label: 'Invoice posting' },
  { value: 'sales_sync', label: 'Sales sync' },
...
function sourceLabel(value?: string) {
  return SOURCE_TYPES.find((s) => s.value === value)?.label || 'Manual entry';
}
                  ...
                  {lines.map((line, i) => (
                    <TableRow key={i} className="border-b border-slate-100 last:border-0">
                      ...
                      <TableCell>
                        <Input
                          type="number"
                          className="h-10 text-right font-mono border-slate-200 rounded-md"
                          value={line.debit || ''}
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-5 sm:p-6 bg-slate-900 rounded-lg">
            <div className="flex justify-between sm:justify-start gap-6 sm:gap-12 font-mono">
              <div className="flex flex-col">
                ...
                <span className="text-lg sm:text-2xl text-white font-semibold">{totalDebit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
              <div className="flex flex-col border-l border-slate-700 pl-6 sm:pl-12">
                ...
                <span className="text-lg sm:text-2xl text-white font-semibold">{totalCredit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
            </div>
            <div className={cn(
...
export default function GeneralJournalTable({ initialEntries, businessId, userId }: { initialEntries: JournalTransaction[], businessId: string, userId: string }) {
  ...
  // Source filter: invoice posting / sales sync / manual / etc.
  ...
  const filteredEntries = useMemo(() => {
    return entries?.filter((e: JournalTransaction) => {
      ...
      const matchesDate = range.from && range.to
        ...
        : true;
      const matchesSource = sourceType === 'all' || e.source_type === sourceType;
      return matchesText && matchesDate && matchesSource;
    });
  }, [entries, filter, range, sourceType]);
  ...
  const SourcePicker = (
    <Select value={sourceType} onValueChange={setSourceType}>
      <SelectTrigger className="h-10 w-[170px] border-slate-200 rounded-md text-sm shrink-0">
        ...
        <div className="relative">
          <Table containerRef={tableRef} onScroll={updateTableScroll} containerClassName="custom-scrollbar">
            ...
            <TableBody>
              ...
              ) : filteredEntries?.map((e: JournalTransaction) => (
