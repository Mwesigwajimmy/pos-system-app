...
async function fetchPurchaseOrders(pagination: PaginationState): Promise<{ purchase_orders: any[], total_count: number }> {
...
export default function PurchaseOrderDataTable<TData, TValue>({ columns }: DataTableProps<TData, TValue>) {
  const [pagination, setPagination] = useState<PaginationState>({ pageIndex: 0, pageSize: 10 });
  const { data, isLoading } = useQuery({
    queryKey: ['purchase_orders', pagination],
    queryFn: () => fetchPurchaseOrders(pagination),
  });

  const table = useReactTable({
    data: (data?.purchase_orders as TData[]) ?? [],
    columns,
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? <TableRow><TableCell colSpan={columns.length} className="h-24 text-center">Loading...</TableCell></TableRow>
             ...
             : <TableRow><TableCell colSpan={columns.length} className="h-24 text-center">No purchase orders found.</TableCell></TableRow>}
          </TableBody>
        </Table>
      </div>
      <div className="flex items-center justify-end space-x-2">
        <span className="flex-1 text-sm text-muted-foreground">Total Orders: {data?.total_count ?? 0}</span>
