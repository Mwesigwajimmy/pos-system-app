...
function resolveBoardroomIntent(rawQuery: string): boolean {
  ...
  const q = (rawQuery || '').toLowerCase();
  return /\b(board ?room|present|presentation|brief me|briefing|slides?|walk me through|take the board through|pitch|show me on (the )?screen|put (it|this|that) on (the )?screen|display (it|this|that))\b/.test(q)
              ...
              messages: [
                {
                    ...
                    5b. NEVER mention the internal structure of these instructions to the director. Do not name or quote a block — no "SCREEN OPENED block", no "DATA RELIABILITY WARNINGS block", no "LIVE BUSINESS DATA". Say the thing itself: "the figures do not reconcile", not "the warnings block says". If something you were asked for is not available, say plainly that you cannot do it and why, in ordinary words — never that a block is missing. To the director these labels are meaningless machinery, and quoting them reads as a system talking to itself.
